...
export default function App() { 
  const [url, setUrl] = useState('https://whatstheweather-cf9ae.web.app/'); 
  ...
  registerNNPushToken(10727, '35ZsWKlQxK1lN5vR4gIdMI'); 
  ...
  useEffect(() => { 
    async function getUnreadNots() { 
      let unreadCount = await getUnreadNotificationInboxCount(10727, '35ZsWKlQxK1lN5vR4gIdMI'); 
  ...
  const handleAnalytics = (path) => { 
    axios.post(`https://app.nativenotify.com/api/analytics`, { 
      app_id: 10727, 
      app_token: '35ZsWKlQxK1lN5vR4gIdMI', 
      screenName: path 
    }); 
  }; 
  
  const handleGoToInbox = async () => { 
    let notifications = await getNotificationInbox(10727, '35ZsWKlQxK1lN5vR4gIdMI'); 
